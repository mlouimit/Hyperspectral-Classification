# This script runs the model training with desired dataset, model, and other parameters.
# It converts the trained model to ONNX format.

# Example usage:
# python training.py --model hamida --dataset IndianPines --training_sample 0.5 --patch_size 5 --epoch 50 --cuda 0
...
for run in range(N_RUNS):
    ...
    if MODEL == 'SVM_grid':
        ...
        try:
            net_dir_name, dataset_dir_name = train(model, optimizer, loss, train_loader, hyperparams['epoch'],
        ...
        probabilities, input_shape = test(model, img, hyperparams)
...
result_dir = f'./checkpoints/{net_dir_name}/{dataset_dir_name}/'
param_dir = result_dir + 'params/'
if not os.path.isdir(param_dir):
    os.mkdir(param_dir)
f = open(param_dir + 'params.pkl','wb')
pickle.dump(hyperparams,f)
f.close()
print(f'Saved parameters to {param_dir}pkl')

### Export to ONNX model
# First choose the checkpoint with the highest accuracy
...
checkpoint_dir = result_dir + onlyfiles[sorted_accs[-1]]
print(f'\nUsing {checkpoint_dir} for export')
# Use this checkpoint to recreate model and export to ONNX
model, _, _, hyperparams = get_model(MODEL, **hyperparams)
model.load_state_dict(torch.load(checkpoint_dir))
dummy_input = torch.rand(input_shape).cuda()
onnx_dir = result_dir + 'onnx/'
if not os.path.isdir(onnx_dir):
    os.mkdir(onnx_dir)
torch.onnx.export(model, dummy_input, onnx_dir + 'model.onnx')
print(f'\nSaved ONNX model to {onnx_dir}model.onnx')
